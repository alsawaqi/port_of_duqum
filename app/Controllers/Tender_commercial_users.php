...
class Tender_commercial_users extends Security_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->access_only_tender("commercial_users", "view");
    ...
    private function _ensure_role(string $title, array $keys): int
{
    ...
    {
        return $this->template->rander("tender_commercial_users/index", [
            "can_create_tender_user" => $this->can_tender("commercial_users", "create"),
        ]);
    }

    public function list_data()
    {
        $list = $this->Tender_commercial_users_model->get_details()->getResult();
        $result = [];
    ...
    public function modal_form()
    {
        ...
        $this->access_only_tender("commercial_users", $id ? "update" : "create");

        $model_info = $id ? $this->Tender_commercial_users_model->get_details(["id" => $id])->getRow() : null;
        ...
        return $this->template->view("tender_commercial_users/modal_form", [
    ...
    public function save()
    {
        ...
        try {
                        $role_id = $this->_ensure_role("Tender Commercial Evaluator", [
                "can_view_tender_commercial_eval",
                "can_update_tender_commercial_eval"
            ]);

            return $this->save_operational_user_assignment(
                $this->Tender_commercial_users_model,
                "tender_commercial_users",
                ["company_id" => $company_id],
                ["job_title" => "Tender Commercial User", "role_id" => $role_id]
            );
        } catch (\Throwable $e) {
    ...
    public function delete()
    {
        ...
        $this->access_only_tender("commercial_users", "delete");
        $id = (int)$this->request->getPost("id");

        $pivot = $this->Tender_commercial_users_model->get_one($id);
        if (!$pivot || (int)$pivot->deleted) {
            return $this->response->setJSON(["success" => false, "message" => app_lang("record_not_found")]);
        }

        $this->db->transStart();
        $this->Tender_commercial_users_model->delete($id);
    ...
    private function _make_row($d)
    {
        ...
        $options = "";
        if ($this->can_tender("commercial_users", "update")) {
            $options .= modal_anchor(get_uri("tender_commercial_users/modal_form"), "<i data-feather='edit' class='icon-16'></i>", [
                "class" => "edit", "title" => app_lang("edit"), "data-post-id" => $d->id
            ]);
        }
        if ($this->can_tender("commercial_users", "delete")) {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", [
                "title" => app_lang("delete"), "class" => "delete", "data-id" => $d->id,
                "data-action-url" => get_uri("tender_commercial_users/delete"), "data-action" => "delete-confirmation"
            ]);
        }
