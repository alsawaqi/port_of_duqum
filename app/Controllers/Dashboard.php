...
class Dashboard extends Security_Controller
{
    ...
    {
        $view_data['dashboards'] = [];

        $dashboards = $this->Dashboards_model->get_details(['user_id' => $this->login_user->id]);
        ...
        // ================================
        // HARD-CODED POD Vendor Dashboard
        // Gate Pass + PTW only
        // ================================
        ...
        $row_widgets = [
            ...
            [
                'columns' => [
                    ['pod_gate_pass_processing_times'],
                    ['pod_ptw_processing_times'],
                ],
                'ratio' => '6-6',
            ],
            [
                'columns' => [
                    ['pod_gate_pass_top_rejection_reasons'],
                    ['pod_ptw_top_rejection_reasons'],
                ],
                'ratio' => '6-6',
            ],
        ];

        $widget_data = $this->_convert_widgets_array_to_formated_obj($row_widgets);
        $view_data['widget_columns'] = $this->make_dashboard($widget_data);
        $view_data['dashboard_id'] = 0;

        // keep this behavior same as staff branch
    ...
    private function _check_widgets_for_staffs()
    {
        ...
        $widget['pod_gate_pass_kpis'] = true;
        $widget['pod_ptw_kpis'] = true;
        $widget['pod_gate_pass_processing_times'] = true;
        $widget['pod_ptw_processing_times'] = true;
        $widget['pod_gate_pass_top_rejection_reasons'] = true;
        $widget['pod_ptw_top_rejection_reasons'] = true;
    ...
    public function save_custom_widget()
    {
        ...
        if ($save_id) {
            ...
            echo json_encode(['success' => true, 'id' => $save_id, 'custom_widgets_row' => $this->_make_widgets_row($custom_widgets_data), 'custom_widgets_data' => $this->_widgets_row_data($custom_widgets_data), 'message' => app_lang('record_saved')]);
        } else {
    ...
    private function _convert_widgets_array_to_formated_obj($row_widgets = [])
    {
        $dashboard_data = [];
    ...
    private function _get_first_row_of_admin_and_team_dashboard($widgets)
    {
        ...
        if (get_array_value($widgets, 'clock_in_out')) {
            $columns[] = ['clock_in_out'];
        }

        $columns[] = ['my_open_tasks'];

        if (get_array_value($widgets, 'events_today')) {
            $columns[] = ['events_today'];
        }
    ...
    private function _get_second_and_third_row_of_admin_and_team_dashboard_widget_columns($widgets)
    {
        ...
        if (get_array_value($widgets, 'all_timesheets_statistics')) {
            $columns[] = ['all_timesheets_statistics'];
        } elseif (get_array_value($widgets, 'my_timesheet_statistics')) {
            $columns[] = ['my_timesheet_statistics'];
        }

        if (get_array_value($widgets, 'estimate_sent_statistics')) {
            $columns[] = ['estimate_sent_statistics'];
        }

        if (get_array_value($widgets, 'invoice_statistics')) {
            $columns[] = ['invoice_statistics'];
        }
    ...
    private function _get_fourth_row_of_admin_and_team_dashboard($widgets)
    {
        ...
        $columns[] = ['project_timeline'];
        if (get_array_value($widgets, 'events') && get_array_value($widgets, 'open_projects_list')) {
            $columns[] = ['events', 'open_projects_list'];
        } elseif (get_array_value($widgets, 'open_projects_list') && get_array_value($widgets, 'starred_projects')) {
            $columns[] = ['open_projects_list', 'starred_projects'];
        }
    ...
    {
        if ($id) {
            return $this->Custom_widgets_model->get_details(['user_id' => $this->login_user->id, 'id' => $id])->getRow();
        }
    }
    ...
    public function save()
    {
        ...
        $data = [
            'user_id' => $this->login_user->id,
            'title' => $this->request->getPost('title'),
            'data' => $dashboard_data ? serialize($dashboard_data) : serialize([]),
        ...
        if ($save_id) {
            echo json_encode(['success' => true, 'dashboard_id' => $save_id, 'message' => app_lang('record_saved')]);
        } else {
            echo json_encode(['success' => false, 'message' => app_lang('error_occurred')]);
        }
    }
    ...
    public function delete_custom_widgets()
    {
        ...
        if ($this->_get_my_custom_widget($id) && $this->Custom_widgets_model->delete($id)) {
            echo json_encode(['success' => true, 'id' => $id, 'message' => app_lang('record_deleted')]);
        } else {
            echo json_encode(['success' => false, 'message' => app_lang('record_cannot_be_deleted')]);
        }
    }
    ...
    private function _get_default_widgets()
    {
        ...
        if ($this->login_user->user_type == 'staff' && $this->show_staff_on_staff) {
            ...
            $default_widgets_array[] = 'pod_gate_pass_kpis';
            $default_widgets_array[] = 'pod_ptw_kpis';
        } else {
            $default_widgets_array = [
                'total_projects',
                'open_projects_list',
    ...
    private function _make_widgets($dashboard_id = 0)
    {
        ...
        // custom widgets
        $custom_widgets = $this->Custom_widgets_model->get_details(['user_id' => $this->login_user->id])->getResult();
        if ($custom_widgets) {
    ...
    private function _widgets_row_data($widget_array)
    {
        ...
        if (is_numeric($key)) {
            $widgets_title = $value;
            $details_button = modal_anchor(get_uri('dashboard/view_custom_widget'), "<i data-feather='more-horizontal' class='icon-16'></i>", ['class' => 'text-off pr10 pl10', 'title' => app_lang('custom_widget_details'), 'data-post-id' => $key]);
        } else {
            $details_button = modal_anchor(get_uri('dashboard/view_default_widget'), "<i data-feather='more-horizontal' class='icon-16'></i>", ['class' => 'text-off pr10 pl10', 'title' => app_lang($key), 'data-post-widget' => $key]);
                ...
                if ($columns) {
                    foreach ($columns as $key => $value) {
                        ...
                        foreach ($value as $content) {
                            ...
                            $view .= $this->_make_widgets_row([$widget_value => get_array_value((array) $content, 'title')], $permissions_array);
                        }
    ...
    private function _get_widgets_for_staffs($widget, $widgets_array)
    {
        if (get_array_value($widgets_array, $widget)) {
            if ($widget == 'clock_in_out') {
                return clock_widget();
            } elseif ($widget == 'events_today') {
                return events_today_widget();
            } elseif ($widget == 'new_posts') {
                return new_posts_widget();
            } elseif ($widget == 'invoice_statistics') {
                return invoice_statistics_widget();
            } elseif ($widget == 'my_timesheet_statistics') {
                return project_timesheet_statistics_widget('my_timesheet_statistics');
            } elseif ($widget == 'ticket_status') {
                $this->init_permission_checker('ticket');

                return ticket_status_widget(['allowed_ticket_types' => $this->allowed_ticket_types, 'show_assigned_tickets_only_user_id' => $this->show_assigned_tickets_only_user_id()]);
            } elseif ($widget == 'timecard_statistics') {
                return timecard_statistics_widget();
            } elseif ($widget == 'income_vs_expenses') {
                return income_vs_expenses_widget('h379', $this->show_own_expenses_only_user_id());
            } elseif ($widget == 'events') {
                return events_widget();
            } elseif ($widget == 'my_open_tasks') {
                return my_open_tasks_widget();
            } elseif ($widget == 'project_timeline') {
                return $this->template->view('dashboards/custom_dashboards/extra_data/widget_with_heading', ['icon' => 'clock', 'widget' => $widget]);
            } elseif ($widget == 'task_status') {
                return my_task_stataus_widget('h377');
            } elseif ($widget == 'sticky_note') {
                return sticky_note_widget('h377');
            } elseif ($widget == 'all_tasks_kanban') {
                return all_tasks_kanban_widget();
            } elseif ($widget == 'todo_list') {
                return todo_list_widget();
            } elseif ($widget == 'open_projects') {
                return open_projects_widget('');
            } elseif ($widget == 'completed_projects') {
                return completed_projects_widget('');
            } elseif ($widget == 'members_clocked_in' || $widget == 'members_clocked_out') {
                $access_attendance = $this->get_access_info('attendance');

                return count_clock_in_out_widget_small([
                    'widget' => $widget,
                    'attendance_access_type' => $access_attendance->access_type,
                    'attendance_allowed_members' => $access_attendance->allowed_members,
                ]);
            } elseif ($widget == 'open_projects_list') {
                return my_open_projects_widget();
            } elseif ($widget == 'starred_projects') {
                return my_starred_projects_widget();
            } elseif ($widget == 'new_tickets' || $widget == 'open_tickets' || $widget == 'closed_tickets') {
                $this->init_permission_checker('ticket');
                $explode_widget = explode('_', $widget);

                return ticket_status_widget_small(['status' => $explode_widget[0], 'allowed_ticket_types' => $this->allowed_ticket_types, 'show_assigned_tickets_only_user_id' => $this->show_assigned_tickets_only_user_id()]);
            } elseif ($widget == 'all_team_members') {
                return all_team_members_widget();
            } elseif ($widget == 'clocked_in_team_members') {
                $this->init_permission_checker('attendance');

                return clocked_in_team_members_widget(['access_type' => $this->access_type, 'allowed_members' => $this->allowed_members]);
            } elseif ($widget == 'clocked_out_team_members') {
                $this->init_permission_checker('attendance');

                return clocked_out_team_members_widget(['access_type' => $this->access_type, 'allowed_members' => $this->allowed_members]);
            } elseif ($widget == 'latest_online_team_members') {
                return active_members_and_clients_widget('staff');
            } elseif ($widget == 'total_invoices' || $widget == 'total_payments' || $widget == 'total_due' || $widget == 'draft_invoices_value') {
                $explode_widget = explode('_', $widget);
            ...
            } elseif ($widget == 'my_tasks_list') {
                return my_tasks_list_widget();
            } elseif ($widget == 'all_timesheets_statistics') {
                return project_timesheet_statistics_widget('all_timesheets_statistics');
            } elseif ($widget == 'pending_leave_approval') {
                $this->init_permission_checker('leave');

                return pending_leave_approval_widget(['access_type' => $this->access_type, 'allowed_members' => $this->allowed_members]);
            } elseif ($widget == 'total_hours_worked' || $widget == 'total_project_hours') {
                return count_total_time_widget_small(0, $widget);
            } elseif ($widget == 'active_members_on_projects') {
                return active_members_on_projects_widget();
            } elseif ($widget == 'draft_invoices') {
                return get_invoices_value_widget('draft_count');
            } elseif ($widget == 'total_clients' || $widget == 'total_contacts' || $widget == 'latest_online_client_contacts') {
                ...
                } elseif ($widget == 'total_contacts') {
                    return total_contacts_widget($show_own_clients_only_user_id, $this->allowed_client_groups);
                } elseif ($widget == 'latest_online_client_contacts') {
                    return active_members_and_clients_widget('client', $show_own_clients_only_user_id, $this->allowed_client_groups);
                }
            } elseif ($widget == 'open_tickets_list') {
                return open_tickets_list_widget();
            } elseif ($widget == 'total_leads') {
                $show_own_leads_only_user_id = $this->show_own_leads_only_user_id();

                return total_leads_widget(true, $show_own_leads_only_user_id);
            } elseif ($widget == 'projects_overview') {
                return projects_overview_widget();
            } elseif ($widget == 'estimate_sent_statistics') {
                return estimate_sent_statistics_widget();
            } elseif ($widget == 'last_announcement') {
                return last_announcement_widget();
            } elseif ($widget == 'team_members_overview') {
                $access_leave = $this->get_access_info('leave');
                $access_attendance = $this->get_access_info('attendance');

                return team_members_overview_widget([
                    'leave_access_type' => $access_leave->access_type,
                    'leave_allowed_members' => $access_leave->allowed_members,
                    'attendance_access_type' => $access_attendance->access_type,
                    'attendance_allowed_members' => $access_attendance->allowed_members,
                ]);
            } elseif ($widget == 'all_tasks_overview') {
                return tasks_overview_widget('all_tasks_overview');
            } elseif ($widget == 'invoice_overview') {
                return invoice_overview_widget();
            } elseif ($widget == 'next_reminder') {
                return next_reminder_widget();
            } elseif ($widget == 'leads_overview') {
                return leads_overview_widget();
            } elseif ($widget == 'my_tasks_overview') {
                return tasks_overview_widget('my_tasks_overview');
            } elseif ($widget == 'pod_gate_pass_kpis') {
                return pod_gate_pass_kpis_widget();
            } elseif ($widget == 'pod_ptw_kpis') {
                return pod_ptw_kpis_widget();
            } elseif ($widget == 'pod_gate_pass_processing_times') {
                return pod_gate_pass_processing_times_widget();
            } elseif ($widget == 'pod_ptw_processing_times') {
                return pod_ptw_processing_times_widget();
            } elseif ($widget == 'pod_gate_pass_top_rejection_reasons') {
                return pod_gate_pass_rejection_reasons_widget();
            } elseif ($widget == 'pod_ptw_top_rejection_reasons') {
                return pod_ptw_rejection_reasons_widget();
            }
    ...
    private function _get_widgets_for_client($widget, $widgets_array)
    {
        ...
        if (get_array_value($widgets_array, $widget)) {
            if ($widget == 'total_projects') {
                return $this->template->view('clients/info_widgets/tab', ['tab' => 'projects', 'client_info' => $client_info]);
            } elseif ($widget == 'total_invoices') {
                return $this->template->view('clients/info_widgets/tab', ['tab' => 'total_invoiced', 'client_info' => $client_info]);
            } elseif ($widget == 'total_payments') {
                return $this->template->view('clients/info_widgets/tab', ['tab' => 'payments', 'client_info' => $client_info]);
            } elseif ($widget == 'total_due') {
                return $this->template->view('clients/info_widgets/tab', ['tab' => 'due', 'client_info' => $client_info]);
            } elseif ($widget == 'open_projects_list') {
                ...
                return ticket_status_widget_small(['status' => $explode_widget[0]]);
            } elseif ($widget == 'project_timeline') {
                return $this->template->view('dashboards/custom_dashboards/extra_data/widget_with_heading', ['icon' => 'clock', 'widget' => $widget]);
            } elseif ($widget == 'open_tickets_list') {
                return open_tickets_list_widget();
    ...
    public function client_default_dashboard()
    {
    ...
    private function _get_client_dashboard_widget_data()
    {
        ...
        if ($client_default_dashboard) {
            ...
            if (get_array_value($widgets, 'total_projects')) {
                $row_1_columns[] = ['total_projects'];
            }
            if (get_array_value($widgets, 'total_invoices')) {
                $row_1_columns[] = ['total_invoices'];
            }
            if (get_array_value($widgets, 'total_payments')) {
                $row_1_columns[] = ['total_payments'];
            }
            if (get_array_value($widgets, 'total_due')) {
                $row_1_columns[] = ['total_due'];
            }

            $ratio = '3-3-3-3';
            if (count($row_1_columns) == 3) {
                $ratio = '4-4-4';
            } elseif (count($row_1_columns) == 2) {
                $ratio = '6-6';
            }
            ...
            if (get_array_value($widgets, 'projects')) {
                $row_2_columns[] = ['projects'];
                $row2['columns'] = $row_2_columns;
                $row2['ratio'] = '12';
            }

            return $this->_convert_widgets_array_to_formated_obj([$row1, $row2]);
        }
    }

    public function edit_client_default_dashboard()
    {
        $this->access_only_admin_or_settings_admin();
        $this->login_user->client_permissions = 'all'; // only for dashboard managment (admin/setting admin)
        $this->show_staff_on_staff = false;
    ...
    public function save_client_default_dashboard()
    {
        $this->access_only_admin_or_settings_admin();

        $dashboard_data = json_decode($this->request->getPost('data'));
        $serialized_data = $dashboard_data ? serialize($dashboard_data) : serialize([]);
    ...
    public function restore_to_default_client_dashboard()
    {
        $this->access_only_admin_or_settings_admin();
    ...
    public function mark_as_default()
    {
