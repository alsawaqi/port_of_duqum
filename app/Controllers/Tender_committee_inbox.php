...
class Tender_committee_inbox extends Security_Controller
{
    ...
    function index()
    {
        ...
        return $this->template->rander("tender_committee_inbox/index");
    }
    ...
    function list_data()
    {
        ...
        $list = $this->Tender_requests_model->get_details([
            "statuses" => ["finance_verified"]
        ])->getResult();

        $result = [];
        foreach ($list as $row) {
            if ($this->can_access_tender_company((int) ($row->company_id ?? 0), "committee")) {
    ...
    function modal_form()
    {
        ...
        $this->require_tender_request_scope($id, "committee");
        ...
        return $this->template->view("tender_committee_inbox/modal_form", [
            "request" => $request,
    ...
    function approve()
    {
        ...
        $id = (int) $this->request->getPost("id");
        $request = $this->require_tender_request_scope($id, "committee");
        ...
        if (($request->status ?? "") !== "finance_verified") {
            return $this->response->setJSON([
                "success" => false,
                "message" => "Only Finance verified requests can be approved by Committee."
            ]);
        }
        ...
        $this->Tender_requests_model->ci_save([
            "committee_approved_by" => $this->login_user->id,
            "committee_approved_at" => date("Y-m-d H:i:s"),
    ...
    function reject()
    {
        ...
        if (($request->status ?? "") !== "finance_verified") {
            return $this->response->setJSON([
                "success" => false,
                "message" => "Only Finance verified requests can be rejected by Committee."
            ]);
        }
        ...
        $this->Tender_requests_model->ci_save([
            "status" => "rejected",
            "committee_reject_comment" => $comment,
            "committee_approved_by" => $this->login_user->id,
            "committee_approved_at" => date("Y-m-d H:i:s"),
        ], $id);
    ...
    private function _make_row($row)
    {
        ...
        $review = modal_anchor(
            get_uri("tender_committee_inbox/modal_form"),
        ...
        $approve = js_anchor(
            ...
            [
                ...
                "class" => "approve",
                "data-id" => $row->id,
                "data-action-url" => get_uri("tender_committee_inbox/approve"),
                "data-action" => "post"
            ]
        );
