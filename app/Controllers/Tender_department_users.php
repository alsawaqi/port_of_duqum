...
class Tender_department_users extends Security_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->access_only_tender("department_users", "view");
    ...
    {
        return $this->template->rander("tender_department_users/index", [
            "can_create_tender_user" => $this->can_tender("department_users", "create"),
        ]);
    }

    public function list_data()
    {
        $list = $this->Tender_department_users_model->get_details()->getResult();
        $result = [];
    ...
    public function modal_form()
    {
        ...
        $this->access_only_tender("department_users", $id ? "update" : "create");

        $model_info = $id ? $this->Tender_department_users_model->get_details(["id" => $id])->getRow() : null;
        ...
        return $this->template->view("tender_department_users/modal_form", [
    ...
    public function save()
    {
        ...
        $role_id = $this->_ensure_role("Tender Department", [
            "can_view_tender_requests",
            "can_create_tender_requests",
    ...
    public function delete()
    {
        ...
        $this->access_only_tender("department_users", "delete");
        $id = (int)$this->request->getPost("id");
    ...
    private function _make_row($d)
    {
        ...
        $options = "";
        if ($this->can_tender("department_users", "update")) {
            $options .= modal_anchor(get_uri("tender_department_users/modal_form"), "<i data-feather='edit' class='icon-16'></i>", [
                "class" => "edit", "title" => app_lang("edit"), "data-post-id" => $d->id
            ]);
        }
        if ($this->can_tender("department_users", "delete")) {
            $options .= js_anchor("<i data-feather='x' class='icon-16'></i>", [
                "title" => app_lang("delete"), "class" => "delete", "data-id" => $d->id,
                "data-action-url" => get_uri("tender_department_users/delete"), "data-action" => "delete-confirmation"
            ]);
        }
