...
use App\Models\Ptw_hmo_users_model;
...
class Ptw_hmo_inbox extends Security_Controller
{
    protected $Ptw_applications_model;
    protected $Ptw_reviews_model;
    protected $Ptw_hmo_users_model;
...
protected $Ptw_attachments_model;
    ...
    public function __construct()
    {
        ...
        $this->Ptw_hmo_users_model = new Ptw_hmo_users_model();
...
$this->Ptw_attachments_model = new Ptw_attachments_model();
    ...
    private function _ensure_hmo_access()
    {
        if ($this->login_user->is_admin) {
            return;
        }

        if (!$this->Ptw_hmo_users_model->is_hmo_user($this->login_user->id)) {
    ...
    {
        $this->_ensure_hmo_access();
        return $this->template->rander("ptw_hmo_inbox/index");
    }
    ...
    public function list_data()
    {
        ...
        if (!$this->login_user->is_admin) {
            $company_ids = $this->Ptw_hmo_users_model->get_active_company_ids($this->login_user->id);
            if (!$company_ids) {
    ...
    {
        $view_btn = anchor(
            get_uri("ptw_hmo_inbox/details/" . $data->id),
    ...
    public function details($id = 0)
    {
        ...
        return $this->template->rander("ptw_hmo_inbox/details", [
    ...
    public function approval_modal_form()
    {
        ...
        if (($application->stage ?? "") !== "hmo") {
            return $this->template->view("errors/html/error_general", [
                ...
                "message" => "Application is not in HMO stage."
            ]);
        }
        ...
        return $this->template->view("ptw_hmo_inbox/approval_modal_form", [
            "application" => $application,
    ...
    public function save_review()
    {
        ...
        if (($application->stage ?? "") !== "hmo") {
            return $this->response->setJSON(["success" => false, "message" => "Application is not in HMO stage."]);
        }
        ...
        $open_review = $this->Ptw_reviews_model->get_open_review($application_id, "hmo")->getRow();
        ...
        if ($open_review) {
            ...
            $revision_no = $this->Ptw_reviews_model->get_next_revision_no($application_id, "hmo");
        ...
        if ($decision === "approved") {
            // Close the review row — approval is final for this stage
            $review_update = [
                "reviewer_id"          => $this->login_user->id,
            ...
            // HMO approved -> move to Terminal, or issue PTW if HSSE skipped Terminal approval.
            $app_update = ptw_hmo_approval_application_update($application, $now);
...
"meta"   => json_encode([
    ...
    "terminal_approval_required" => $application->terminal_approval_required ?? 1,
    "terminal_skipped" => $decision === "approved" && !ptw_terminal_approval_is_required($application),
]),
    ...
    public function review_history_modal()
    {
        ...
        return $this->template->view("ptw_hmo_inbox/review_history_modal", [
            "application" => $application,
            "reviews" => $reviews
        ]);
    }
    ...
    private function _can_act_on_application($application): bool
    {
        ...
        return $this->Ptw_hmo_users_model->has_active_company_assignment(
            $this->login_user->id,
            $company_id
        );
    }
