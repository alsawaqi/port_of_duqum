...
use App\Models\Ptw_hsse_users_model;
...
class Ptw_hsse_inbox extends Security_Controller
{
    protected $Ptw_applications_model;
    protected $Ptw_reviews_model;
    protected $Ptw_hsse_users_model;
...
protected $Ptw_attachments_model;
    ...
    public function __construct()
    {
        ...
        $this->Ptw_hsse_users_model = new Ptw_hsse_users_model();
...
$this->Ptw_attachments_model = new Ptw_attachments_model();
    ...
    private function _ensure_hsse_access()
    {
        if ($this->login_user->is_admin) {
            return;
        }

        if (!$this->Ptw_hsse_users_model->is_hsse_user($this->login_user->id)) {
    ...
    {
        $this->_ensure_hsse_access();
        return $this->template->rander("ptw_hsse_inbox/index");
    }
    ...
    public function list_data()
    {
        ...
        if (!$this->login_user->is_admin) {
            $company_ids = $this->Ptw_hsse_users_model->get_active_company_ids($this->login_user->id);
            if (!$company_ids) {
    ...
    {
        $view_btn = anchor(
            get_uri("ptw_hsse_inbox/details/" . $data->id),
    ...
    public function approval_modal_form()
    {
        ...
        if (($application->stage ?? "") !== "hsse") {
            return $this->template->view("errors/html/error_general", [
                ...
                "message" => "Application is not in HSSE stage."
            ]);
        }
        ...
        return $this->template->view("ptw_hsse_inbox/approval_modal_form", [
            "application" => $application,
    ...
    public function save_review()
    {
        ...
        if (($application->stage ?? "") !== "hsse") {
            return $this->response->setJSON(["success" => false, "message" => "Application is not in HSSE stage."]);
        }
        ...
        $open_review = $this->Ptw_reviews_model->get_open_review($application_id, "hsse")->getRow();
        ...
        if ($open_review) {
            ...
            $revision_no = $this->Ptw_reviews_model->get_next_revision_no($application_id, "hsse");
        ...
        if ($decision === "approved") {
            ...
            // Stay in HSSE stage and wait for contractor resubmission.
            $app_update = [
                "updated_at"   => $now,
                "stage"        => "hsse",
                "status"       => "revise",
    ...
    public function review_history_modal()
    {
        ...
        return $this->template->view("ptw_hsse_inbox/review_history_modal", [
            "application" => $application,
            "reviews" => $reviews
        ]);
    }
    ...
    private function _can_act_on_application($application): bool
    {
        ...
        return $this->Ptw_hsse_users_model->has_active_company_assignment(
            $this->login_user->id,
            $company_id
        );
    }
