...
function can_tender_3key_opening() {
    ...
    /**
     * Whether the user has a specific Gate Pass Master permission (view/create/update/delete for a section).
     ...
     */
    function can_gate_pass($section, $action) {
        if ($this->is_admin()) {
            return true;
        }
    ...
    /**
     * Whether the user has a specific PTW Master permission (view/create/update/delete for a section).
     * Sections: hsse_users, hmo_users, terminal_users, request_list, hazard_documents, ppe, preparation, reasons.
     */
    function can_ptw($section, $action) {
        if ($this->is_admin()) {
            return true;
        }
