<div id="page-content" class="page-wrapper clearfix gp-details-page">
    ...
    if ($status === "approved" || $status === "department_approved") $status_class = "gp-badge-soft-success";
    if ($status === "rejected")  $status_class = "gp-badge-soft-danger";
    if ($status === "issued")    $status_class = "gp-badge-soft-success";

    // stage classes (optional mapping)
    ...
    <div class="card gp-card mb15">
                ...
                <div class="title-button-group gp-actions-row">
                    <?php if ($show_review_btn): ?>
                        <?php echo modal_anchor(
                            get_uri("gate_pass_department_requests/approval_modal_form"),
                            "<i data-feather='check-square' class='icon-16'></i> " . app_lang("department_review"),
                            ["class" => "btn btn-primary gp-btn-primary", "title" => app_lang("department_review"), "data-post-id" => $request->id]
                        ); ?>
                    <?php endif; ?>
        ...
        <div class="p20 pt10">
            ...
            <!-- Optional: workflow stepper (nice touch) -->
    ...
    <div id="gp-dept-visitors-card" class="card gp-card mb15">
        ...
        <div class="table-responsive p15 p-sm-10 gp-table-wrap">
            <table id="gp-dept-visitors-table" class="display" width="100%" cellspacing="0"></table>
        </div>
    </div>

    <!-- Vehicles -->
    <div id="gp-dept-vehicles-card" class="card gp-card mb15">
        <div class="gp-section-title gp-section-title-row">
            <div class="d-flex align-items-center">
        ...
        <div class="table-responsive p15 p-sm-10 gp-table-wrap">
            <table id="gp-dept-vehicles-table" class="display" width="100%" cellspacing="0"></table>
        </div>
    </div>
</div>

<style>
/* ===== CRITICAL FIX: Enable vertical scrolling for entire page ===== */
.gp-details-page#page-content {
...
.main-scrollable-page:has(.gp-details-page) {
  overflow-y: auto !important;
  overflow-x: hidden;
  height: auto !important;
  min-height: 100vh;
...
.page-container .main-scrollable-page:has(.gp-details-page) {
  padding-bottom: 24px;
}

/* ===== Premium Gate Pass UI ===== */
.gp-card { 
  border-radius: 14px; 
  border: 1px solid rgba(0,0,0,.06); 
  overflow: hidden; /* Changed from visible to hidden for card structure */
...
/* ===== FIXED: Table scrolling - horizontal only, no vertical trap ===== */
.gp-table-wrap {
...
/* Override DataTables scroll settings */
.gp-table-wrap .dataTables_wrapper {
  overflow: visible !important;
}

.gp-table-wrap .dataTables_scroll {
  overflow: visible !important;
}

.gp-table-wrap .dataTables_scrollBody {
  overflow-x: auto !important;
  overflow-y: visible !important;
...
/* Ensure table cells don't create scroll containers */
.gp-table-wrap td,
.gp-table-wrap th {
  white-space: nowrap;
}
...
/* Section cards: allow scroll-into-view for tables */
#gp-dept-visitors-card { scroll-margin-top: 20px; }
#gp-dept-vehicles-card { scroll-margin-top: 20px; }
...
/* Responsive */
@media (max-width: 1200px) { .gp-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); } }
@media (max-width: 576px) {
    .gp-grid { grid-template-columns: 1fr; }
    .gp-header { padding: 16px; }
    .gp-section-title { padding: 14px 16px; }
    .gp-actions-row { flex-direction: column; }
    .gp-actions-row .btn { width: 100%; }
    .gp-stepper { gap: 4px; }
    .gp-step { padding: 4px 6px; }
}
</style>
...
$(document).ready(function () {
    ...
    // Initialize visitors table with scrolling fix
    $("#gp-dept-visitors-table").appTable({
        source: "<?php echo_uri('gate_pass_department_requests/visitors_list_data/'); ?>" + requestId,
        scrollY: false, // Disable vertical scroll on DataTables
        ...
        columns: [
            ...
            { title: "<?php echo app_lang('attachments'); ?>", data: 9, class: "text-center", orderable: false }
        ],
        order: [[0, "asc"]]
    });

    // Initialize vehicles table with scrolling fix
    $("#gp-dept-vehicles-table").appTable({
        source: "<?php echo_uri('gate_pass_department_requests/vehicles_list_data/'); ?>" + requestId,
        scrollY: false, // Disable vertical scroll on DataTables
        scrollCollapse: false,
        columns: [
    ...
    // Additional fix: Force remove any DataTables scroll containers after initialization
    setTimeout(function() {
        $('.gp-table-wrap .dataTables_scrollBody').css({
            'overflow-y': 'visible',
            'max-height': 'none',
            'height': 'auto'
        });
