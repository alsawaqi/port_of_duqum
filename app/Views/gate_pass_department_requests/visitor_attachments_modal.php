<?php
// Template extracts view data, so $visitor is passed directly from controller
if (empty($visitor)) {
    echo "<p class='p15 text-off'>" . app_lang("record_not_found") . "</p>";
    return;
}
// Ensure we have an object (getRow() may return object or array depending on driver)
if (is_array($visitor)) {
                    ...
                    <?php foreach ($attachment_fields as $field => $label): ?>
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <?php
                                $view_url = get_uri("gate_pass_department_requests/visitor_attachment_download/" . $visitor->id . "/" . $field);
