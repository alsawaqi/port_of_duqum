<?php echo form_open(get_uri("ptw_hsse_inbox/save_review"), ["id" => "ptw-hsse-review-form", "class" => "general-form", "role" => "form"]); ?>
...
<div class="modal-body clearfix">
    ...
    <div class="form-group">
        ...
        <select name="decision" id="ptw-hsse-decision" class="form-control" required>
    ...
    <div class="form-group" id="ptw-hsse-status-reason-wrap" style="display:none;">
        <label><?php echo app_lang("status_change_reason"); ?> <span class="text-danger">*</span></label>
        <textarea name="status_change_reason" id="ptw-hsse-status-reason" class="form-control" rows="3"></textarea>
...
$(document).ready(function () {
    function syncDecisionUi() {
        var decision = $("#ptw-hsse-decision").val();
    ...
    $("#ptw-hsse-decision").on("change", syncDecisionUi);
    syncDecisionUi();

    // Auto-fill reason textarea when a reason is picked from the dropdown
    $("#ptw-hsse-reason-id").on("change", function () {
        var selected = $(this).find("option:selected").text().trim();
        var current  = $("#ptw-hsse-status-reason").val().trim();
        if ($(this).val() !== "0" && current === "") {
            $("#ptw-hsse-status-reason").val(selected);
        }
        if ($(this).val() === "0") {
            $("#ptw-hsse-status-reason").val("");
        }
    ...
    $("#ptw-hsse-review-form").appForm({
    onSuccess: function () {
        // Always return reviewer to main inbox after action
        window.location.href = "<?php echo get_uri('ptw_hsse_inbox'); ?>";
    }
});
});
</script>