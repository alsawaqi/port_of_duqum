...
class Tender_department_manager_users_model extends Crud_model
{
    protected $table = null;

    public function __construct()
    {
        $this->table = "tender_department_manager_users";
        parent::__construct($this->table);
    }

    public function get_details($options = [])
    {
        $t = $this->db->prefixTable("tender_department_manager_users");
        ...
        if (isset($options["department_id"]) && $options["department_id"] !== "") $where .= " AND $t.department_id=" . (int)$options["department_id"];
        ...
        $sql = "SELECT $t.*,
                ...
                LEFT JOIN $departments ON $departments.id=$t.department_id AND $departments.deleted=0
                $where
                ORDER BY $t.id DESC";
