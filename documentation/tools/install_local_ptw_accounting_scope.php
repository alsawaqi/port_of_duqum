...
final class LocalPtwAccountingScopeBootstrap extends CodeIgniter\Boot
{
...
$locked = false;
try {
    LocalPtwAccountingScopeBootstrap::load($paths); if (!defined("CI_DEBUG")) { define("CI_DEBUG", true); }
    ...
    $migrations = [
        ['2026_08_03_100000', 'ptw_application_company_scope', 'Ptw_application_company_scope'],
    ...
    if (!$apply) { exit(0); }
    $locked = (int) $db->query("SELECT GET_LOCK('pod_local_ptw_accounting_scope_upgrade', 0) AS acquired")->getRow()->acquired === 1;
    ...
    foreach (['ptw_applications'] as $table) {
        echo $db->prefixTable($table) . ': ' . $db->table($table)->countAllResults() . ' rows.' . PHP_EOL;
    }
    echo 'PTW company scope installed and exact unique company names backfilled. No fees, users or payment records were changed.' . PHP_EOL;
} catch (Throwable $error) {
    // Do not print driver messages, configuration or connection credentials.
    fwrite(STDERR, 'PTW scope installation stopped: ' . get_class($error) . ' (code ' . $error->getCode() . '). Check the local configuration and migration schema.' . PHP_EOL);
    exit(1);
} finally {
    if ($db && $locked) { $db->query("SELECT RELEASE_LOCK('pod_local_ptw_accounting_scope_upgrade')"); }
}
